...
const BATCH_SIZE = Math.max(1, Math.min(5, Number(process.env.HISTORY_BATCH_SIZE || 3)));
const BETWEEN_BATCH_MS = Math.max(0, Number(process.env.HISTORY_BATCH_DELAY_MS || 1800));
const BETWEEN_SYMBOL_MS = Math.max(0, Number(process.env.HISTORY_SYMBOL_DELAY_MS || 500));
...
function selectEntries(entries) {
  ...
  if (MODE === 'sample') return entries.filter((entry) => entry.sample !== false).slice(0, 10);
  if (MODE === 'full_backfill' || MODE === 'incremental') return entries;
...
async function processSymbol(entry, localReference, approvedRecords) {
  ...
  const range = MODE === 'incremental' ? '3mo' : '1y';
  const audit = {
    ticker: entry.ticker,
    operation: MODE,
    startedAt,
...
async function main() {
  ...
  console.log(`V12.2 Historical 100: mode=${MODE}, symbols=${entries.length}, batchSize=${BATCH_SIZE}`);
  for (let offset = 0; offset < entries.length; offset += BATCH_SIZE) {
    ...
    if (offset + BATCH_SIZE < entries.length && BETWEEN_BATCH_MS > 0) await sleep(BETWEEN_BATCH_MS);
  }

  const oldAudit = readJson(path.join(DATA_DIR, 'source-audit.json'), { operations: [] });
  const operations = [...(oldAudit.operations || []), ...results.map((result) => result.audit)].slice(-1000);
  ...
  const summary = buildSummary(REPO_ROOT, allEntries, {
    yahoo: { status: results.some((result) => result.ok) ? 'available' : 'failed', role: 'historical backfill' },
    ...
    investing: { status: 'manual_fallback_in_sample', role: 'fallback/manual verification' },
  });
